...
class api {
     ...
     * @param string|null $sectionavailability Course section availability JSON.
     * @param int $sectionvisible Course section visible flag.
     * @return array Availability fields consumed by the dashboard.
     */
    private static function module_availability_meta(
        ?string $availability,
        int $visible,
        array $modulewindow = [],
        ?string $sectionavailability = null,
        int $sectionvisible = 1
    ): array {
        list($availablefrom, $availableuntil) = self::availability_window($availability);
        list($sectionfrom, $sectionuntil) = self::availability_window($sectionavailability);
        if ($sectionfrom) {
            $availablefrom = max($availablefrom, $sectionfrom);
        }
        if ($sectionuntil) {
            $availableuntil = $availableuntil ? min($availableuntil, $sectionuntil) : $sectionuntil;
        }
        $duedate = !empty($modulewindow['duedate']) ? (int)$modulewindow['duedate'] : 0;
        // Modules such as quizzes expose their effective deadline as a close
        // date instead of a field literally named "duedate".
        if (!$duedate) {
            foreach (['timeclose', 'closedate', 'cutoffdate'] as $deadlinefield) {
                if (!empty($modulewindow[$deadlinefield])) {
                    $duedate = (int)$modulewindow[$deadlinefield];
                    break;
                }
            }
        }
    ...
    private static function build_cm_name_map(int $courseid): array {
        ...
        $cms = $DB->get_records_sql("
            SELECT cm.id, m.name AS modname, cm.instance, cm.visible, cm.availability, cm.deletioninprogress,
                   cs.visible AS sectionvisible, cs.availability AS sectionavailability
                ...
                foreach ($instances as $cmid => $cm) {
                    ...
                    if (isset($names[$instanceid])) {
                        $availability = self::module_availability_meta(
                            ...
                            $cm->sectionavailability ?? null,
                            (int)($cm->sectionvisible ?? 1)
                        );
                        $map[$cmid] = [
     ...
     */
    /** Build student rows when the course has no usable grade items. */
    private static function course_last_accesses(int $courseid, array $studentids): array {
        global $DB;
        if (empty($studentids)) {
            return [];
        }
        list($studentsql, $studentparams) = $DB->get_in_or_equal(
            array_map('intval', $studentids), SQL_PARAMS_NAMED, 'lastaccessusr'
        );
        $records = $DB->get_records_sql(
            "SELECT userid AS id, timeaccess
               FROM {user_lastaccess}
              WHERE courseid = :lastaccesscourseid
                AND userid $studentsql",
            array_merge(['lastaccesscourseid' => $courseid], $studentparams)
        );
        $lastaccesses = [];
        foreach ($records as $record) {
            $lastaccesses[(int)$record->id] = (int)$record->timeaccess;
        }
        return $lastaccesses;
    }

    /** Return student IDs with native academic activity, using bulk course queries. */
    private static function course_academic_evidence(int $courseid, array $studentids): array {
        global $DB;
        if (empty($studentids)) {
            return [];
        }
        ...
        $queries = [
            [
                "SELECT DISTINCT cmc.userid
                   FROM {course_modules_completion} cmc
                   JOIN {course_modules} cm ON cm.id = cmc.coursemoduleid
                  WHERE cm.course = :evidencecompletioncourse
                    AND cmc.completionstate > 0",
                ['evidencecompletioncourse' => $courseid],
                'evidencecompletionusr',
                'cmc.userid',
            ],
            [
                "SELECT DISTINCT s.userid
                  ...
                  WHERE a.course = :evidenceassigncourse
                    AND s.status = :evidenceassignstatus
                    AND s.latest = 1",
                ['evidenceassigncourse' => $courseid, 'evidenceassignstatus' => 'submitted'],
                'evidenceassignusr',
                's.userid',
            ],
            [
                "SELECT DISTINCT qa.userid
                  ...
                  WHERE q.course = :evidencequizcourse
                    AND qa.state = :evidencequizstate",
                ['evidencequizcourse' => $courseid, 'evidencequizstate' => 'finished'],
                'evidencequizusr',
                'qa.userid',
            ],
            [
                "SELECT DISTINCT p.userid
                  ...
                  WHERE f.course = :evidenceforumcourse",
                ['evidenceforumcourse' => $courseid],
                'evidenceforumusr',
                'p.userid',
            ],
        ];
        foreach ($queries as [$sql, $params, $prefix, $useridfield]) {
            list($studentsql, $studentparams) = $DB->get_in_or_equal(
                array_map('intval', $studentids), SQL_PARAMS_NAMED, $prefix
            );
            $userids = $DB->get_fieldset_sql(
                $sql . " AND $useridfield $studentsql",
                array_merge($params, $studentparams)
            );
            foreach ($userids as $userid) {
                $evidence[(int)$userid] = true;
            }
        }
        return $evidence;
    }
    ...
    /** Build student rows when the course has no usable grade items. */
    private static function grade_roster_rows(int $courseid, array $students): array {
        global $DB;
        if (empty($students)) {
            return [];
        }
        $studentids = array_map('intval', array_keys($students));
        list($studentsql, $studentparams) = $DB->get_in_or_equal($studentids, SQL_PARAMS_NAMED, 'rosterusr');
        $enrolmentrows = $DB->get_records_sql(
            "SELECT ue.userid AS id, MIN(ue.timecreated) AS enrolmentcreated
               FROM {user_enrolments} ue
               JOIN {enrol} e ON e.id = ue.enrolid
              WHERE e.courseid = :rostercourseid
                AND ue.userid $studentsql
                AND e.status = 0
                AND ue.status = 0
           GROUP BY ue.userid",
            array_merge(['rostercourseid' => $courseid], $studentparams)
        );
        $lastaccesses = self::course_last_accesses($courseid, $studentids);
        $academicevidence = self::course_academic_evidence($courseid, $studentids);
        $rows = [];
        foreach ($students as $student) {
            $rows[] = [
                ...
                'Email' => $student->email,
                '_enrolment_created' => (int)($enrolmentrows[(int)$student->id]->enrolmentcreated ?? 0),
                '_last_access' => $lastaccesses[(int)$student->id] ?? 0,
                '_academic_evidence' => !empty($academicevidence[(int)$student->id]) ? 1 : 0,
                'Course total (Grade)' => null,
                'Course total max (Grade)' => null,
                'User ID' => (int)$student->id,
                'Picture URL' => self::user_picture_url($student),
            ];
        }
        return $rows;
    }
    ...
    /**
     * Get the current enrolled-student roster and available grades for a course.
     *
     * @param int $courseid The course ID.
    ...
    public static function get_grades(int $courseid, int $groupid = 0): array {
        ...
        $assignedstudentids = $DB->get_fieldset_sql(
            "SELECT DISTINCT ra.userid
               FROM {role_assignments} ra
              WHERE ra.contextid = :studentcontextid
                AND ra.roleid $rolesql",
            array_merge(['studentcontextid' => (int)$context->id], $roleparams)
        );
        ...
        if (empty($allitems)) {
            return self::grade_roster_rows($courseid, $students);
        }
        ...
        foreach ($cmrows as $row) {
            ...
            $availability = self::module_availability_meta(
                ...
                $row->sectionavailability ?? null,
                (int)($row->sectionvisible ?? 1)
            );
        ...
        $items = array_values(array_filter($items, function($item) {
            ...
            $name = \core_text::strtolower((string)$item->get_name(false));
            $isglobalforumgrade = strpos($name, 'global') !== false
                && (strpos($name, 'forum') !== false || strpos($name, 'fórum') !== false);
            return !$isglobalforumgrade;
        }));
        $itemsbycmid = [];
        foreach ($items as $item) {
            $cmmeta = $cmindex[$item->itemmodule][(int)$item->iteminstance] ?? null;
            $cmid = is_array($cmmeta) ? (int)($cmmeta['cmid'] ?? 0) : 0;
            if ($cmid <= 0 || isset($itemsbycmid[$cmid])) {
                continue;
            }
            $itemsbycmid[$cmid] = $item;
        }
        $items = array_values($itemsbycmid);
        if (empty($items)) {
            return self::grade_roster_rows($courseid, $students);
        }
        // Maximum value of all gradeable activities currently included in the course.
        // This is different from Course total max, which only sums items already graded
        // for each student and therefore represents the partial maximum launched so far.
        $coursefullmax = 0.0;
        foreach ($items as $item) {
            $coursefullmax += (float)($item->grademax ?: 100);
        }
        ...
        // 3. Bulk load of all grades (1 query) for the roster loaded above.
        ...
        if (!empty($itemidbycmid)) {
            ...
            foreach ($completionrows as $completionrow) {
                ...
                if ($itemid) {
                    $userid = (int)$completionrow->userid;
                    $activitystatemap[$itemid][$userid] = max(
                        (int)($activitystatemap[$itemid][$userid] ?? 0),
                        (int)$completionrow->completionstate
                    );
                }
            }
        }
        // H5P and Game are attempt-based activities. A recorded grade confirms
        // the attempt even when the achieved score is exactly zero.
        foreach ($items as $item) {
            $itemid = (int)$item->id;
            $module = $itemmodulebyid[$itemid] ?? '';
            if (!in_array($module, ['h5pactivity', 'hvp', 'game'], true)) {
                continue;
            }
            foreach ($students as $student) {
                $userid = (int)$student->id;
                $studentgrades = $grademap[$userid] ?? [];
                if (array_key_exists($itemid, $studentgrades) && $studentgrades[$itemid] !== null) {
                    $activitystatemap[$itemid][$userid] = 1;
                }
            }
        }
        $assignitems = [];
        ...
        if (!empty($assignitems)) {
            // For assignments, course completion does not prove that a submission
            // exists. Discard the generic completion state and rebuild the current
            // delivery map exclusively from live submitted assignment records.
            foreach ($assignitems as $assignitemid) {
                unset($activitystatemap[$assignitemid]);
            }
        ...
        foreach ($students as $stu) {
            $row = [
                ...
                '_last_access' => $lastaccesses[(int)$stu->id] ?? 0,
                '_academic_evidence' => !empty($academicevidence[(int)$stu->id]) ? 1 : 0,
            ];
            $total    = 0.0;
            $totalmax = 0.0;
            $hasgrade = false;
            ...
            $row['Course full max (Grade)']  = $coursefullmax > 0 ? round($coursefullmax, 1) : null;
    ...
    private static function translate_component(string $comp): string {
        $map = [
            ...
            'mod_board'       => 'Board',
            'mod_journal'     => 'Journal',
            'mod_vpl'         => 'Virtual programming lab',
            'mod_bigbluebuttonbn' => 'Virtual classroom',
            'core'            => 'System',
        ];
