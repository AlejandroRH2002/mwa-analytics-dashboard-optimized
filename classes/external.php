...
class external extends \external_api {
    ...
    private static function restore_students_in_ai_response(int $courseid, string $text): string {
        ...
        foreach ($users as $student) {
            ...
            $alias = self::get_ai_alias($courseid, (int)$student->id);
            // Providers frequently typographically transform the ASCII hyphen in
            // Student-001 into a non-breaking hyphen, en dash, em dash or minus.
            // Match the numeric alias independently of that presentation change so
            // no privacy alias leaks into the local Moodle interface.
            if (preg_match('/^Student-(\d{3})$/', $alias, $matches)) {
                $number = preg_quote($matches[1], '/');
                $pattern = '/(?<![\p{L}\p{N}])Student(?:[\s\p{Pd}\x{2212}])*' . $number . '(?![\p{L}\p{N}])/ui';
