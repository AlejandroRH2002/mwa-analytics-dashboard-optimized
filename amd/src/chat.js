...
define(['block_mwa_dashboard/dashboardstore', 'core/log', 'core/templates'], function(Store, Log, Templates) {
    ...
    function localizeMoodleTerms(value, language){
        var label=norm(value);
    ...
    /** Inline markdown: bold, italic and inline code. Input must already be escaped. */
    function inlineMd(s){
        return s
    ...
    /**
     * Renders the assistant's markdown as real elements. The model tends to
     * answer with headings, rules and bullet lists, and previously only bold
     * was handled, so the raw "##", "---" and "-" markers leaked into the
     * bubble. Escaping happens first, so nothing here can inject HTML.
     */
    function richText(value){
        var lines = esc(value).split('\n');
        var out = [];
        var listTag = '';
        function closeList(){
        ...
        function openList(tag){
            if(listTag !== tag){ closeList(); out.push('<' + tag + ' class="chat-md-list">'); listTag = tag; }
        }
        lines.forEach(function(raw){
            var t = raw.trim();
            if(!t){ closeList(); return; }
            if(/^(-{3,}|\*{3,}|_{3,})$/.test(t)){ closeList(); out.push('<hr class="chat-md-hr">'); return; }
            var h = t.match(/^(#{1,6})\s*(.*)$/);
            if(h){
                closeList();
                out.push('<div class="chat-md-h chat-md-h' + h[1].length + '">' + inlineMd(h[2]) + '</div>');
                return;
            }
            var ol = t.match(/^\d+[.)]\s+(.*)$/);
            if(ol){ openList('ol'); out.push('<li>' + inlineMd(ol[1]) + '</li>'); return; }
            var ul = t.match(/^[-*+]\s+(.*)$/);
            if(ul){ openList('ul'); out.push('<li>' + inlineMd(ul[1]) + '</li>'); return; }
            closeList();
            out.push('<p class="chat-md-p">' + inlineMd(t) + '</p>');
        });
        closeList();
        return out.join('');
    }
    ...
    function buildContext(){
        ...
        /* The roster must come from the dashboard state, which merges the logs
           ...
           every student who never accessed Moodle - precisely the ones the
           teacher most needs the assistant to talk about. */
        var dashboardStudents=state.students||[];
        var students=[];var stuSet={};
        function addStudent(n){
            n=norm(n);
            if(n&&!stuSet[n]){stuSet[n]=1;students.push(n);}
        }
        dashboardStudents.forEach(function(s){addStudent(s.name);});
        data.forEach(function(r){addStudent(r.nomecompleto);});
        var la={};
        ...
        var now=new Date();
        /* Math.floor and a null for "never accessed", matching EngagementCalc.
           The old version rounded and returned 99, so the chat disagreed with
           the dashboard by a day and invented a fake 99-day figure. */
        function daysAgo(d){if(!d)return null;return Math.max(0,Math.floor((now-d)/86400000));}
        // grades rows: {__mwa_type__:'student','First name':...,'Last name':...,'Course total (Grade)':N}
        var gradeMap={};
        var gradeDetailMap={}; // student name -> {actName: value}
        var actNamesRow=grades&&grades[0]&&grades[0]['__mwa_type__']==='activity_names'?grades[0]:null;
        grades.forEach(function(g){
            if(!g||g['__mwa_type__']!=='student')return;
            var fn=norm((g['First name']||'')+(g['Last name']?' '+g['Last name']:''));
            if(!fn)return;
            var total=g['Course total (Grade)'];
            if(total!=null&&total!=='-')gradeMap[fn]=Number(total);
            // Per-activity grades
            if(actNamesRow){
                gradeDetailMap[fn]={};
                Object.keys(actNamesRow).forEach(function(k){
                    if(k==='__mwa_type__'||/^act_(cmid|module|available|availablefrom|availableuntil|duedate)_/.test(k))return;
                    var actName=actNamesRow[k];
                    var val=g[k];
                    if(actName&&val!=null&&val!=='-')gradeDetailMap[fn][actName]=val;
                });
            }
        });
        ...
        /* Engagement scores come straight from the dashboard state, which is
           where the shared EngagementCalc result is already stored. */
        var evData=state.students||[];
        var atRisk=0;
        /* Caps keep the prompt within budget. Every list built from them is
           reported to the model with its real total, so it never presents a
           truncated list as the whole picture. */
        var STUDENT_CAP=200;
        var ACTIVITY_CAP=60;
        var summaries=students.slice(0,STUDENT_CAP).map(function(n){
            ...
            /* Same threshold as the Action Center KPI, so the assistant and the
               dashboard never report a different number of at-risk students. */
            if(score!=null&&score<=40)atRisk++;
            var actGrades=gradeDetailMap[n]||{};
            var actGradeStr=Object.keys(actGrades).map(function(k){return k+': '+actGrades[k];}).join(', ');
            return {
                nome:n,
        ...
        var dashboardActivities=state.activities||[];
        function low(v){return norm(v).toLowerCase();}
        function localizedEventLabel(value){
            var label=norm(value);
        ...
        function sameName(a,b){return low(a)===low(b);}
        function matchActivity(a,b){
            var aa=low(a),bb=low(b);
            return aa&&bb&&(aa===bb||aa.indexOf(bb)>=0||bb.indexOf(aa)>=0);
        }
        function studentModel(name){
            var ln=low(name);
            return dashboardStudents.find(function(s){return sameName(s.name,name)||low(s.email)===ln;})||null;
        }
        function logActivityName(r){
            return localizedEventLabel(r.contextodoevento||r.context||r.eventcontext||r._resource||r.nomedoevento);
        }
        function logIsSubmission(r){
            var text=low([r.nomedoevento,r.action,r.componente,r.component,r._modtype].join(' '));
            return text.indexOf('submit')>=0||text.indexOf('submitted')>=0||text.indexOf('submission')>=0||
                text.indexOf('upload')>=0||text.indexOf('graded')>=0||text.indexOf('submetid')>=0||
                text.indexOf('tentativa enviada')>=0||text.indexOf('attempt submitted')>=0||
                text.indexOf('h5p completion')>=0||text.indexOf('completed')>=0;
        }
        var submittedByStudent={};
        var accessedByStudent={};
        data.forEach(function(r){
            var sn=norm(r.nomecompleto);
            var an=logActivityName(r);
            if(!sn||!an)return;
            accessedByStudent[sn]=accessedByStudent[sn]||{};
            accessedByStudent[sn][low(an)]=an;
            if(logIsSubmission(r)){
                submittedByStudent[sn]=submittedByStudent[sn]||{};
                submittedByStudent[sn][low(an)]=an;
            }
        });

        // Activities: names and access counts from logs
        // Build activity map using full name from the event context field
        // Key = lowercase for dedup, but preserve original name
        var actMap={};
        data.forEach(function(r){
            // Prefer event context field (real activity name) over event name field (action label)
            var rawName=(r.contextodoevento&&r.contextodoevento.length>2)?r.contextodoevento:(r.nomedoevento||'');
            rawName=localizedEventLabel(rawName);
            rawName=rawName.trim();
            if(!rawName||rawName.length<2)return;
            // Skip generic module-type names (assign, forum, quiz, page, etc. without spaces)
            if(rawName.indexOf(' ')<0&&rawName.length<10)return;
            var key=rawName.toLowerCase();
            if(!actMap[key])actMap[key]={name:rawName,type:(r._modtype||'').toLowerCase()||'outro',accessed:0,students:{}};
            // Update name if longer (more descriptive)
            if(rawName.length>actMap[key].name.length)actMap[key].name=rawName;
            actMap[key].accessed++;
            if(r.nomecompleto)actMap[key].students[norm(r.nomecompleto)]=1;
        });
        // Also enrich from MWA_ACT_NAMES (grade items)
        var actNamesMap=w.MWA_ACT_NAMES||{};
        var gradedMeta=[];
        if(actNamesRow){
            Object.keys(actNamesRow).forEach(function(k){
                ...
                if(!m)return;
                var seq=m[1];
                var name=norm(actNamesRow[k]);
                if(!name)return;
                gradedMeta.push({
                    nome:name,
                    modulo:norm(actNamesRow['act_module_'+seq]||''),
                    cmid:parseInt(actNamesRow['act_cmid_'+seq]||0,10)||0,
                    disponivel:String(actNamesRow['act_available_'+seq]||'1')!=='0',
                    abreEm:parseInt(actNamesRow['act_availablefrom_'+seq]||0,10)||0,
                    entregaEm:parseInt(actNamesRow['act_duedate_'+seq]||0,10)||0,
                    fechaEm:parseInt(actNamesRow['act_availableuntil_'+seq]||0,10)||0
                });
            });
        }
        var gradedActivities=gradedMeta.length?gradedMeta.map(function(a){return a.nome;}):Object.values(actNamesMap).filter(function(n){return n&&String(n).length>2;});

        var allStudentNames=students.slice(0,STUDENT_CAP);
        // Activity coverage follows the activity cards: only students with at least one Moodle interaction
        // are part of the denominator. Students with zero interactions are not counted in coverage.
        var interactingStudentNames=allStudentNames.filter(function(n){
            return data.some(function(r){return norm(r.nomecompleto)===n;});
        });
        var activities=Object.values(actMap)
            .sort(function(a,b){return b.accessed-a.accessed;})
            .slice(0,ACTIVITY_CAP)
            .map(function(a){
                var isGraded=gradedActivities.some(function(g){
                    var gn=norm(String(g)).toLowerCase();var an=norm(a.name).toLowerCase();
                    return gn===an||(gn.length>4&&(gn.indexOf(an)>=0||an.indexOf(gn)>=0));
                });
                var studentsWhoAccessed=Object.keys(a.students);
                return {
                    nome:a.name,
                    tipo:a.type,
                    acessos:a.accessed,
                    alunosUnicos:studentsWhoAccessed.length,
                    cobertura:Math.round((studentsWhoAccessed.length/Math.max(interactingStudentNames.length,1))*100),
                    faltam:Math.max(interactingStudentNames.length-studentsWhoAccessed.length,0),
                    temNota:isGraded,
                };
            });

        // Use the exact activity/resource dataset used by the Activities tab.
        // This prevents the chat from independently reconstructing access coverage
        // and guarantees that CMID, unique students, missing students and coverage
        // match the activity cards.
        if (window.MWAActivities && typeof window.MWAActivities.getChatSnapshot === 'function') {
            var cardActivities = window.MWAActivities.getChatSnapshot();
            if (Array.isArray(cardActivities) && cardActivities.length) {
                activities = cardActivities.map(function(a) {
                    var isGraded = gradedMeta.some(function(g) {
                        if (a.cmid && g.cmid) return Number(a.cmid) === Number(g.cmid);
                        return matchActivity(g.nome, a.nome);
                    });
                    return {
                        nome: a.nome,
                        tipo: a.tipo,
                        acessos: a.acessos,
                        alunosUnicos: a.alunosUnicos,
        ...
        summaries.forEach(function(s){
            var ds=studentModel(s.nome);
            var actGrades=gradeDetailMap[s.nome]||{};
            var submitted=submittedByStudent[s.nome]||{};
            var accessed=accessedByStudent[s.nome]||{};
            var pendingActivities=[];
            var pendingGrades=[];
            var deliveredActivities=[];
            gradedActivities.forEach(function(a){
                var hasGrade=Object.keys(actGrades).some(function(k){return matchActivity(k,a);});
                var hasSubmission=Object.keys(submitted).some(function(k){return matchActivity(k,a);});
                if(hasSubmission||hasGrade)deliveredActivities.push(a);
                if(!hasSubmission&&!hasGrade)pendingActivities.push(a);
                if(hasSubmission&&!hasGrade)pendingGrades.push(a);
            });
            var resourcesNotAccessed=activities.filter(function(a){
                return !a.temNota&&!Object.keys(accessed).some(function(k){return matchActivity(k,a.nome);});
            }).map(function(a){return a.nome;});
            if(ds){
                /* Dashboard values win over the locally recomputed ones so the
                   assistant quotes exactly what the teacher sees on screen. */
                if(ds.interactions!=null)s.interacoes=ds.interactions;
                if(ds.daysWithoutAccess!=null)s.diasSemAcesso=ds.daysWithoutAccess;
                else if(ds.last==null)s.diasSemAcesso='never accessed';
                if(ds.grade!=null)s.notaTotal=Number(ds.grade).toFixed(1);
                s.score=ds.score!=null?Math.round(ds.score):null;
                s.cobertura=ds.coverage!=null?Math.round(ds.coverage):null;
                s.conclusao=ds.completion!=null?Math.round(ds.completion):null;
                s.diasAtivos=ds.activeDays!=null?ds.activeDays:null;
                s.itensComNota=ds.gradeItems||gradedActivities.length;
                s.notasLancadas=ds.gradeLaunched||Object.keys(actGrades).length;
            }else{
                s.score=s.engScore&&s.engScore!=='-'?Number(String(s.engScore).replace('%','')):null;
                s.cobertura=null;
                s.conclusao=null;
                s.diasAtivos=null;
                s.itensComNota=gradedActivities.length;
                s.notasLancadas=Object.keys(actGrades).length;
            }
            s.atividadesEntregues=deliveredActivities;
            s.atividadesPendentes=pendingActivities;
            s.notasPendentes=pendingGrades;
            s.conteudosNaoAcessados=resourcesNotAccessed;
        });

        // Peak access hour
        var hourCount={};
        data.forEach(function(r){if(r._date)hourCount[r._date.getHours()]=(hourCount[r._date.getHours()]||0)+1;});
        var peakHour=Object.keys(hourCount).sort(function(a,b){return hourCount[b]-hourCount[a];})[0];
        var modelStudents=dashboardStudents.length?dashboardStudents:summaries;
        atRisk=modelStudents.filter(function(s){return s.score!=null&&Number(s.score)<=40;}).length;
        var withoutRecentAccess=modelStudents.filter(function(s){
            if(s.daysWithoutAccess!=null)return Number(s.daysWithoutAccess)>=7;
            if(s.diasSemAcesso==='never accessed')return true;
            return s.last==null;
        }).length;
        var withInteractions=modelStudents.filter(function(s){return Number(s.interactions||s.interacoes||0)>0;}).length;
        var approved=modelStudents.filter(function(s){
            var g=s.grade!=null?s.grade:(gradeMap[s.nome]!=null?gradeMap[s.nome]:null);
            return g!=null&&Number(g)>=60;
        }).length;
        function avgField(list,field){
            var vals=list.map(function(s){return Number(s[field]);}).filter(function(v){return !isNaN(v);});
            return vals.length?Math.round(vals.reduce(function(a,b){return a+b;},0)/vals.length):null;
        }
        var lowCoverageActivities=activities.filter(function(a){return a.cobertura<60;});
        var gradedPendingTotal=summaries.reduce(function(total,s){return total+(s.atividadesPendentes?s.atividadesPendentes.length:0);},0);

        return {
            curso:{
                nomeCurso:cfg.coursename||tr('chat_unknown_course'),
                totalAlunos:students.length,
                alunosComInteracoes:withInteractions,
                totalInteracoes:data.length,
                mediaInteracoesPorAluno:Math.round(data.length/Math.max(withInteractions,1)),
                alunosEmRisco:atRisk,
                alunosSemAcessoRecente:withoutRecentAccess,
                notaMedia:avgGrade,
                aprovados:approved,
                mediaCobertura:avgField(modelStudents,'coverage'),
                mediaConclusao:avgField(modelStudents,'completion'),
            ...
            estudantes:summaries.map(function(s){
                return {
                    nomeLocal:s.nome,
                    nota:s.notaTotal,
                    interacoes:s.interacoes,
                    diasSemAcesso:s.diasSemAcesso,
                    engajamento:s.score,
                    cobertura:s.cobertura,
                    conclusao:s.conclusao,
                    diasAtivos:s.diasAtivos,
                    atividadesPendentes:s.atividadesPendentes||[],
                    notasPendentes:s.notasPendentes||[],
                    atividadesEntregues:s.atividadesEntregues||[],
                    conteudosNaoAcessados:s.conteudosNaoAcessados||[]
                };
            }),
            totais:{
                alunos:students.length,
                atividades:Object.keys(actMap).length,
                atividadesListadas:activities.length,
                recursosBaixaCobertura:lowCoverageActivities.length,
            },
        };
    }
    ...
    function renderSidebar(){
        ...
        renderTemplate(el, 'chat_sidebar', {
            emptylabel: tr('chat_no_convs'),
            deletelabel: tr('chat_delete_conv'),
            conversations: CONVS.slice(0,15).map(function(c){
                return {
                    id: c.id,
                    title: c.title.slice(0,26),
                    date: new Date(c.createdAt).toLocaleDateString(undefined,{day:'2-digit',month:'2-digit'}),
                    active: c.id===CUR_ID,
                    deletelabel: tr('chat_delete_conv')
                };
            })
    ...
    function updateContextChips(){
        ...
        var chips=[];
        if(cfg.ia_enabled){
            var provider=cfg.ia_provider||'IA';
