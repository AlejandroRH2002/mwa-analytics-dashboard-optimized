...
define([
    ...
    'core/chartjs',
    'core/templates'
], function(Store, ActionCenter, Alerts, ClassList, Profile, Activities, Grades, Interventions, Chat, TeacherFeedback,
    EngagementCalc, Chart, Templates) {
    ...
    var init = function(config) {
        ...
        function renderActionGreeting() {
          ...
          var greeting = tr(key).replace('{name}', name || tr('teacher_label'));
          var courseName = String(cfg.coursename || '').trim();
          el.textContent = '';
          el.appendChild(document.createTextNode(greeting + ' '));
          if (courseName) {
            el.appendChild(document.createTextNode(tr('action_welcome_course') + ' '));
            var courseStrong = document.createElement('strong');
            courseStrong.textContent = courseName;
            el.appendChild(courseStrong);
            el.appendChild(document.createTextNode('.'));
          } else {
            el.appendChild(document.createTextNode(tr('action_welcome_message')));
          }
        }
        ...
        function setupCustomSelects() {
          var root = document.getElementById('block-mwa-dashboard-app');
          if (!root || root.dataset.mwaCustomSelects === '1') { return; }
          root.dataset.mwaCustomSelects = '1';

          function closeAll(except) {
            root.querySelectorAll('.mwa-select-shell.is-open').forEach(function(shell) {
              if (shell !== except) {
                shell.classList.remove('is-open');
                var toggle = shell.querySelector('.mwa-select-toggle');
          ...
          function enhance(select) {
            if (!select || select.dataset.mwaSelectEnhanced === '1' || select.multiple || select.hasAttribute('data-mwa-native-select')) {
              return;
            }
            select.dataset.mwaSelectEnhanced = '1';
            select.classList.add('mwa-select-native');

            var shell = document.createElement('div');
            shell.className = 'mwa-select-shell';
            var toggle = document.createElement('button');
            toggle.type = 'button';
            toggle.className = 'mwa-select-toggle';
            toggle.setAttribute('aria-haspopup', 'listbox');
            toggle.setAttribute('aria-expanded', 'false');
            var menu = document.createElement('div');
            menu.className = 'mwa-select-menu';
            ...
            function rebuild() {
              ...
              Array.prototype.forEach.call(select.options, function(option) {
                ...
                item.type = 'button';
                item.className = 'mwa-select-option';
            ...
            toggle.addEventListener('click', function(event) {
              ...
              rebuild();
              var opening = !shell.classList.contains('is-open');
              closeAll(shell);
              shell.classList.toggle('is-open', opening);
              toggle.setAttribute('aria-expanded', opening ? 'true' : 'false');
              if (opening) {
                var active = menu.querySelector('.is-selected:not(:disabled)') || menu.querySelector('.mwa-select-option:not(:disabled)');
                if (active) { setTimeout(function() { active.focus(); }, 0); }
              }
            });
            ...
            menu.addEventListener('keydown', function(event) {
              var enabled = Array.prototype.slice.call(menu.querySelectorAll('.mwa-select-option:not(:disabled)'));
              ...
              if (event.key === 'Escape') {
                ...
                var next = index < 0 ? 0 : (index + direction + enabled.length) % enabled.length;
          ...
          new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
              if (mutation.target && mutation.target.matches && mutation.target.matches('select') &&
                  mutation.target.dataset.mwaSelectEnhanced === '1') {
          ...
          document.addEventListener('click', function(event) {
            if (!event.target.closest('.mwa-select-shell')) { closeAll(); }
          });
        }
        setupCustomSelects();
        document.addEventListener('click', function(e){
          var helpTipTarget = e.target.closest('.mwa-help-tip');
          if (helpTipTarget) {
            e.preventDefault();
            e.stopPropagation();
            document.querySelectorAll('.mwa-help-tip.is-open').forEach(function(tip) {
              if (tip !== helpTipTarget) tip.classList.remove('is-open');
            });
            helpTipTarget.classList.toggle('is-open');
            return;
          }
          document.querySelectorAll('.mwa-help-tip.is-open').forEach(function(tip) {
          ...
          if (btn) {
            ...
            var classFilter = btn.getAttribute('data-cl-filter-target') || btn.getAttribute('data-cl-filter');
            var actLow = btn.getAttribute('data-act-low') || btn.classList.contains('ac-item-more');
            showPage(page);
            ...
            if (page === 'activities' && actLow && Modules.Activities && typeof Modules.Activities.filterLowEngagement === 'function') {
              Modules.Activities.filterLowEngagement();
            }
          }
        });
        document.addEventListener('keydown', function(e) {
          ...
          var keyboardHelpTip = e.target.closest('.mwa-help-tip');
          if (keyboardHelpTip) {
            e.preventDefault();
            keyboardHelpTip.click();
            return;
          }
        ...
        function buildModel(){
          ...
          state.grades.forEach(function(g){
            ...
            by[key].enrolmentCreated=parseInt(g._enrolment_created||0,10)||0;
            by[key].moodleLastAccess=parseInt(g._last_access||0,10)||0;
            by[key].nativeAcademicEvidence=String(g._academic_evidence||'0')==='1';
            by[key].gradeRow=g;
          });
          state.students=Object.keys(by).map(function(k){
            ...
            if(s.moodleLastAccess>0){
              var moodleLastDate=new Date(s.moodleLastAccess*1000);
              if(!s.last||moodleLastDate>s.last)s.last=moodleLastDate;
            }
            if(!s.last&&s.enrolmentCreated){
              s.daysWithoutAccess=Math.max(0,Math.floor((Date.now()/1000-s.enrolmentCreated)/86400));
            }
            s.grade=calc.grade;
            s.gradeMax=calc.gradeMax;
            s.hasAcademicEvidence=!!s.nativeAcademicEvidence||Object.keys(s.gradeRow||{}).some(function(field){
              return /^act_current_\d+$/.test(field)&&Number(s.gradeRow[field]||0)>0;
            });
            s.hasCourseActivity=Number(s.interactions||0)>0||s.hasAcademicEvidence;
            s.score=calc.score;
        ...
        function kpiSmart(label, value, sub, delta, invertDelta, tip, options) {
          ...
          if (options.classFilter) {
            attrs += ' data-cl-filter="' + esc(options.classFilter) + '"';
          }
          if (options.url) {
          ...
          var icon = options.icon || '<svg class="mwa-ui-icon" aria-hidden="true"><use href="#mwa-icon-chart"></use></svg>';
          var deltaValue = (delta === null || delta === undefined || isNaN(Number(delta))) ? 0 : Number(delta);
          if (options.nonDecreasing && deltaValue < 0) { deltaValue = 0; }
          deltaValue = Math.round(deltaValue * 10) / 10;
          var deltaClass = deltaValue > 0 ? 'is-up' : deltaValue < 0 ? 'is-down' : 'is-zero';
          var deltaArrow = deltaValue > 0 ? '&#8593;' : deltaValue < 0 ? '&#8595;' : '';
          var deltaNumber = Math.abs(deltaValue);
          var deltaText = deltaValue === 0 ? '0' : (deltaValue > 0 ? '+' : '-') + String(deltaNumber).replace('.', ',');
          var deltaChip = '<span class="ac-kpi-delta '+deltaClass+'">'
            + (deltaArrow ? '<span class="ac-kpi-delta-arrow" aria-hidden="true">'+deltaArrow+'</span>' : '')
            + '<span>'+deltaText+'</span></span>';
          return '<div class="kpi ac-main-kpi ac-kpi-' + esc(theme) + clickable + attrs + '>'
            + '<div class="ac-kpi-head"><span class="ac-kpi-icon" aria-hidden="true">'+icon+'</span>'
            + '<div class="kpi-label"><span>'+label+helpTip(tip)+'</span></div></div>'
            + deltaChip
        ...
        function renderKPIs(){
          // Action Center KPIs must follow the raw Moodle log loaded by the dashboard.
          // Each new log row is one new interaction. Do not depend on the rebuilt
          // student counters here, because those can be cached/normalised separately.
          var totalStudentInteractions = (state.logs || []).length;
          var logStudentNames = {};
          (state.logs || []).forEach(function(log) {
            var name = norm(log.nomecompleto || log.student_name || log.name || log.fullname || '');
            if (name) { logStudentNames[name] = true; }
          });
          var loggedCourseStudents = Object.keys(logStudentNames).length;
          var activeCourseStudents = state.students.filter(function(student){return student.hasCourseActivity;}).length;
          var avgInt = loggedCourseStudents ? Math.round((totalStudentInteractions / loggedCourseStudents) * 10) / 10 : 0;
          var atRisk = state.students.filter(function(s){return s.score<=40;}).length;
          var grades = state.students.map(function(s){return s.grade;}).filter(function(v){return v!==null && isFinite(Number(v));}).map(Number);
          var avgGrade = grades.length ? Math.round((grades.reduce(function(a,b){return a+b;},0)/grades.length) * 10) / 10 : null;
          var cfg = Store.getConfig ? Store.getConfig() : (config || {});
          ...
          // Single source of truth for both the visible KPI values and trend chips.
          var currentKpis = {
            activeStudents: state.students.length,
            interactingStudents: activeCourseStudents,
            totalInteractions: totalStudentInteractions,
            averageInteractions: avgInt,
            averageGrade: avgGrade === null ? 0 : avgGrade
          };

          
          // Action Center KPI trend: mirror the post-intervention progress pattern.
          // Calculate an immutable BEFORE value and compare it with the CURRENT value.
          // For interaction KPIs, reconstruct the BEFORE value from the Moodle log itself,
          // so a new event can never overwrite or reset the comparison anchor.
          var now = new Date();
          var dayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
          var studentNames = {};
          state.students.forEach(function(student) {
            var n = lower(student.name || student.fullname || '');
            if (n) { studentNames[n] = true; }
          });
          var beforeLogs = [];
          (state.logs || []).forEach(function(log) {
            var dt = parseDate(log);
            if (dt && dt < dayStart) { beforeLogs.push(log); }
          });
          function uniqueStudentsInLogs(logs) {
            var seen = {};
            logs.forEach(function(log) {
              var name = norm(log.nomecompleto || log.student_name || log.name || log.fullname || '');
              var key = lower(name);
              // Count only students loaded in the course when that roster is available.
              if (key && (!Object.keys(studentNames).length || studentNames[key])) { seen[key] = true; }
            });
            return Object.keys(seen).length;
          }
          var beforeTotalInteractions = beforeLogs.length;
          var beforeInteractingStudents = uniqueStudentsInLogs(beforeLogs);
          var beforeAvgInteractions = beforeInteractingStudents ? Math.round((beforeTotalInteractions / beforeInteractingStudents) * 10) / 10 : 0;

          // Active students and grade do not have enough historical value in a log row to
          // reconstruct their previous numeric KPI. Keep only those two anchors persisted.
          var trendCourseId = courseid || (function() {
            try { return parseInt(new URL(window.location.href).searchParams.get('id') || '0', 10) || 0; } catch (e) { return 0; }
          })();
          var trendKey = 'mwa_action_kpi_nonlog_' + String(trendCourseId);
          var today = now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0');
          var savedTrend = null;
          try { savedTrend = window.localStorage ? JSON.parse(window.localStorage.getItem(trendKey) || 'null') : null; } catch (e) { savedTrend = null; }
          var nonLogBaseline = savedTrend && savedTrend.day === today && savedTrend.counts ? savedTrend.counts : null;
          if (!nonLogBaseline && trendCourseId > 0 && state.students.length > 0) {
            nonLogBaseline = {activeStudents: currentKpis.activeStudents, interactingStudents: currentKpis.interactingStudents, averageGrade: currentKpis.averageGrade};
            try { if (window.localStorage) { window.localStorage.setItem(trendKey, JSON.stringify({day: today, counts: nonLogBaseline})); } } catch (e) {}
          }
          nonLogBaseline = nonLogBaseline || {activeStudents: currentKpis.activeStudents, interactingStudents: currentKpis.interactingStudents, averageGrade: currentKpis.averageGrade};
          if (nonLogBaseline.interactingStudents === undefined || nonLogBaseline.interactingStudents === null) {
            nonLogBaseline.interactingStudents = currentKpis.interactingStudents;
            try { if (window.localStorage && trendCourseId > 0) { window.localStorage.setItem(trendKey, JSON.stringify({day: today, counts: nonLogBaseline})); } } catch (e) {}
          }
          function diff(current, before, nonDecreasing) {
            current = Number(current); before = Number(before);
            if (!isFinite(current) || !isFinite(before)) { return 0; }
            var value = Math.round((current - before) * 10) / 10;
            return nonDecreasing && value < 0 ? 0 : value;
          }
          var dActiveStudents = diff(currentKpis.activeStudents, nonLogBaseline.activeStudents, false);
          var dStudents = diff(currentKpis.interactingStudents, nonLogBaseline.interactingStudents, true);
          var dLogs = diff(currentKpis.totalInteractions, beforeTotalInteractions, true);
        ...
        function mwaChartType(label){
          /* Exact activity/resource type using the same taxonomy as the activities view. */
        ...
        function goToChartBucket(label){
          ...
          if (Modules.Activities && Modules.Activities.setFilter) {
            /* The activities control groups modules visually but applies the exact selected type. */
            var btn = document.getElementById(isVideo ? 'actTabVideo' : 'actTabActivity');
            Modules.Activities.setFilter(type, btn);
          }
        }
        function chartModuleLabel(label){
          var c = String(label || '').toLowerCase().trim();
          var map = {
            ...
            wiki: tr('event_type_wiki','Wiki'),
            choice: tr('event_type_choice','Choice'),
            feedback: tr('event_type_feedback','Feedback'),
            survey: tr('event_type_survey','Survey'),
            questionnaire: tr('event_type_questionnaire','Questionnaire'),
            game: tr('event_type_game','Game'),
            workshop: tr('event_type_workshop','Workshop'),
            attendance: tr('event_type_attendance','Attendance'),
            board: tr('event_type_board','Board'),
            journal: tr('event_type_journal','Journal'),
            vpl: tr('event_type_vpl','Virtual lab'),
            bigbluebuttonbn: tr('event_type_webconf','Web conference'), webconf: tr('event_type_webconf','Web conference')
          };
          return map[c] || label;
        }
        ...
        function hmActivityUrl(l, resourceName) {
          ...
          var map = {
            assign:'assign', assignment:'assign', forum:'forum', quiz:'quiz', page:'page',
            resource:'resource', file:'resource', url:'url', scorm:'scorm',
            h5pactivity:'h5pactivity', hvp:'hvp', book:'book', folder:'folder', imscp:'imscp',
            data:'data', glossary:'glossary', chat:'chat', choice:'choice', choicegroup:'choicegroup',
            groupselect:'groupselect', feedback:'feedback', survey:'survey', questionnaire:'questionnaire',
            game:'game', workshop:'workshop', lesson:'lesson', wiki:'wiki', attendance:'attendance',
            board:'board', journal:'journal', vpl:'vpl', bigbluebuttonbn:'bigbluebuttonbn',
            webconf:'webconf'
          };
        ...
        function renderHeatmap() {
          [
            ['heatmapInsights', 'lightbulb'],
            ['heatmapGradeCorr', 'chart'],
            ['heatmapBestTime', 'clock']
          ].forEach(function(item) {
            var body = document.getElementById(item[0]);
            var head = body && body.parentElement ? body.parentElement.querySelector('.card-head') : null;
            if (!head || head.querySelector('.mwa-ui-icon')) return;
            Array.prototype.slice.call(head.childNodes).forEach(function(node) {
              if (node.nodeType === 3) node.nodeValue = node.nodeValue.replace(/^\s*\S+\s*/, '');
            });
            var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
            svg.setAttribute('class', 'mwa-ui-icon');
            svg.setAttribute('aria-hidden', 'true');
            var use = document.createElementNS('http://www.w3.org/2000/svg', 'use');
            use.setAttribute('href', '#mwa-icon-' + item[1]);
            svg.appendChild(use);
          ...
          if (dateWrap && !document.getElementById('heatmapClearFilters')) {
            var clearFilters = document.createElement('button');
            clearFilters.type = 'button';
            clearFilters.id = 'heatmapClearFilters';
            clearFilters.className = 'hm-clear-filters';
            var clearIcon = document.createTextNode('\uD83D\uDDD1 ');
            var clearSpan = document.createElement('span');
            clearSpan.textContent = tr('clearfilters');
            clearFilters.appendChild(clearIcon);
            clearFilters.appendChild(clearSpan);
            clearFilters.addEventListener('click', function() {
              if (searchInput) searchInput.value = '';
              if (selStudent) selStudent.value = '';
              if (selMod) selMod.value = '';
              if (selMode) selMode.value = 'access';
              ['heatmapStartDate', 'heatmapEndDate'].forEach(function(id) {
                var input = document.getElementById(id);
                if (input) input.value = '';
              });
              dateWrap.querySelectorAll('[data-hm-range]').forEach(function(button) {
                var selected = button.getAttribute('data-hm-range') === '7';
                button.className = selected ? 'btn-accent active' : 'btn-ghost';
              });
              window.MWAHeatmapDetailState = null;
              renderHeatmap();
            });
            dateWrap.appendChild(clearFilters);
          }
          ...
          logs.forEach(function(l) {
            ...
            if (mode === 'access') {
              ...
              if (!lastGrid[uid] || d > lastGrid[uid].date) {
                lastGrid[uid] = {date: d, key: key, log: l};
              }
            }
          ...
          if (mode === 'dropout') {
            ...
            Object.keys(lastGrid).forEach(function(uid) {
              ...
              cellBuckets[k].push(lastGrid[uid].log);
            });
          }
          var visibleLogs = mode === 'dropout'
            ? Object.keys(lastGrid).map(function(uid) { return lastGrid[uid].log; }).filter(Boolean)
            : logs;
          ...
          function renderHmDetail(title, list, extra) {
            ...
            var resources = hmTopResources(list);
            ...
            var activitiesHtml = '<details class="hm-name-section hm-activity-section"><summary><span>' + tr('hm_detail_activities') + '</span><strong>' + resources.length + '</strong><em>' + tr('hm_view_activities') + '</em></summary><div class="hm-resource-list">' + resourceHtml + '</div></details>';
            var body = '<div class="card-head"><span>' + esc(title) + '</span></div>'
              ...
              + '<div class="hm-detail-grid"><div><div class="hm-detail-section-head"><h4>' + tr('hm_detail_students') + '</h4></div>' + studentHtml + '<button type="button" class="btn-accent hm-message-students" data-hm-message="1">' + tr('hm_msg_this_time') + '</button></div>'
              + '<div><h4>' + tr('hm_detail_activities') + '</h4>' + activitiesHtml + '</div></div>'
              + '</div>';
          ...
          function hmRestoreDetail() {
            ...
            if (saved && saved.kind === 'filtered') {
              renderHmDetail(tr('hm_filtered_action'), visibleLogs, {});
              return;
            }
        ...
        function animateDashboard(){
          ...
          if(!root)return;
          var selector='.kpi-value,.fr-kpi-value,.fr-delta b,.act-card-stat-val,.sp-kpi-value,.cl-kpi-value,.cl-kpi-card>strong,.hm-kpi-val,.sa-kpi-val,.int2-motivo-count,.int2-summary-row>strong';
          root.querySelectorAll(selector).forEach(function(node){
            if(node.dataset.mwaCounting==='1')return;
            var raw=String(node.textContent||'').trim();
            var match=raw.match(/^(-?\d+(?:[.,]\d+)?)(\s*%)?$/);
            if(!match)return;
            var decimal=match[1].indexOf(',')>=0?',':'.';
            var decimals=(match[1].split(/[.,]/)[1]||'').length;
            var target=parseFloat(match[1].replace(',','.'));
            if(!isFinite(target))return;
            var suffix=match[2]||'';
            var animationKey=String(target)+'|'+suffix;
            if(node.dataset.mwaAnimated===animationKey)return;
            node.dataset.mwaAnimated=animationKey;
            node.dataset.mwaCounting='1';
            var step=0;
            var steps=40;
            node.textContent=(decimals?(0).toFixed(decimals):'0')+suffix;
            var timer=setInterval(function(){
              ...
              if(decimal===',')formatted=formatted.replace('.',',');
              node.textContent=formatted+suffix;
              if(progress>=1){clearInterval(timer);delete node.dataset.mwaCounting;}
            },35);
          });
          root.querySelectorAll('.lines path,.fr-journey-area,.fr-reason-bar').forEach(function(node){
            if(node.dataset.mwaAnimated==='1')return;
            node.dataset.mwaAnimated='1';
            node.classList.remove('mwa-chart-motion');
        ...
        function showPage(p){updateDashboardTimestamp();try{var currentUrl=new window.URL(window.location.href);currentUrl.searchParams.set('mwa_page',p);window.history.replaceState({},'',currentUrl.toString());}catch(ignore){}document.querySelectorAll('.page').forEach(function(x){x.classList.remove('active')});var page=$('page-'+p);if(page)page.classList.add('active');document.querySelectorAll('.nav-item').forEach(function(x){x.classList.toggle('active',x.getAttribute('data-page')===p)});if(p==='ac'&&Modules.ActionCenter){setTimeout(function(){Modules.ActionCenter.renderEngagement();},120);}if(p==='alerts'&&Modules.Alerts&&state.logs.length){Modules.Alerts.reset();Modules.Alerts.render();}if(p==='classlist'&&Modules.ClassList&&(state.logs.length||state.grades.length)){Modules.ClassList.render();}if(p==='studentprofile'&&Modules.Profile){if(typeof window.MWAReloadData==='function'&&!window.MWAProfileReloading){window.MWAProfileReloading=true;window.MWAReloadData().then(function(){Modules.Profile.render();}).catch(function(){Modules.Profile.render();}).then(function(){window.MWAProfileReloading=false;});}else{Modules.Profile.render();}}if(p==='activities'&&Modules.Activities&&state.logs.length){Modules.Activities.render();}if(p==='grades'&&Modules.Grades){Modules.Grades.render();}if(p==='interventions'&&Modules.Interventions){if(typeof window.MWAReloadData==='function'&&!window.MWAInterventionsReloading){window.MWAInterventionsReloading=true;window.MWAReloadData().then(function(){Modules.Interventions.render();}).catch(function(){Modules.Interventions.render();}).then(function(){window.MWAInterventionsReloading=false;});}else{Modules.Interventions.render();}}if(p==='teacherfeedback'&&Modules.TeacherFeedback){Modules.TeacherFeedback.render();}if(p==='chat'&&Chat){Chat.render();var ensure=Modules.Chat&&Modules.Chat.ensureData?Modules.Chat.ensureData:window.MWAEnsureDashboardData;if(typeof ensure==='function'){Promise.resolve(ensure()).then(function(){Chat.render();}).catch(function(error){console.error('[MWA Chat] Automatic tab data load failed:',error);});}}if(p==='heatmap'&&state.logs.length){renderHeatmap();['heatmapStudentSel','heatmapModSel','heatmapModeSel'].forEach(function(id){var el=document.getElementById(id);if(el&&!el.dataset.hmInit){el.dataset.hmInit='1';el.addEventListener('change',renderHeatmap);}});}}
        function refreshDashboardHome(){showPage('ac');var main=document.querySelector('.block-mwa-dashboard-app .main');if(main)main.scrollTop=0;if(typeof window.MWAReloadData==='function'&&!window.MWAHomeReloading){window.MWAHomeReloading=true;window.MWAReloadData(true).catch(function(){}).then(function(){window.MWAHomeReloading=false;});}else{renderAll();}}
        window.MWARefreshDashboardHome=refreshDashboardHome;
        ...
        function receiveData(data){try{
          ...
          // omitted it or returned an empty placeholder. That was making Active students,
          // Student/Interaction and Grade average disappear after a new interaction.
          var incomingLogs = JSON.parse(data.logs || '[]');
          while(Array.isArray(incomingLogs)&&incomingLogs.length===1&&Array.isArray(incomingLogs[0])){incomingLogs=incomingLogs[0];}
          if(Array.isArray(incomingLogs) && incomingLogs.length){
            // Moodle logs are append-only for this dashboard. If a refresh returns only a
            // recent slice, merge it with the already loaded history instead of replacing it.
            if(Array.isArray(state.logs) && state.logs.length && incomingLogs.length < state.logs.length){
              var seenLogs = {};
              function logKey(l){
                return [l.id||l._id||'',l._ts||l.timecreated||'',l._userid||l.userid||'',l.eventname||l.nomedoevento||'',l.contextinstanceid||l._cmid||'',l.hora||''].join('|');
              }
              state.logs.forEach(function(l){seenLogs[logKey(l)]=true;});
              incomingLogs.forEach(function(l){var k=logKey(l);if(!seenLogs[k]){state.logs.push(l);seenLogs[k]=true;}});
            }else{
              state.logs=incomingLogs;
            }
          }else if(!Array.isArray(state.logs)){
            state.logs=[];
          }

          var incomingGrades = JSON.parse(data.grades || '[]');
          while(Array.isArray(incomingGrades)&&incomingGrades.length===1&&Array.isArray(incomingGrades[0])){incomingGrades=incomingGrades[0];}
          if(Array.isArray(incomingGrades) && incomingGrades.length){
            // A log-only refresh may send just the activity_names metadata row. That row is
            // not the course roster and must never replace the previously loaded student/grade rows.
            var incomingStudentGradeRows = incomingGrades.filter(function(g){return g && g.__mwa_type__ !== 'activity_names';});
            var incomingMetaRows = incomingGrades.filter(function(g){return g && g.__mwa_type__ === 'activity_names';});
            var existingStudentGradeRows = Array.isArray(state.grades) ? state.grades.filter(function(g){return g && g.__mwa_type__ !== 'activity_names';}) : [];
            if(incomingStudentGradeRows.length){
              state.grades=incomingMetaRows.concat(incomingStudentGradeRows);
            }else if(existingStudentGradeRows.length){
              // Preserve roster, grades and enrolment state while only logs are being refreshed.
              var existingMetaRows = Array.isArray(state.grades) ? state.grades.filter(function(g){return g && g.__mwa_type__ === 'activity_names';}) : [];
              state.grades=(incomingMetaRows.length ? incomingMetaRows : existingMetaRows).concat(existingStudentGradeRows);
            }else{
              state.grades=incomingGrades;
            }
        ...
        Store.setHandler('showPage', function(page) {
          ...
          if (page === 'ac' && Modules.ActionCenter) {
            renderKPIs();
            Modules.ActionCenter.renderEngagement();
            Modules.ActionCenter.render();
          }
          showPage(page);
          if (refreshActionCenter && typeof window.MWAReloadData === 'function' && !window.MWAActionCenterReloading) {
            window.MWAActionCenterReloading = true;
            window.MWAReloadData().then(function() {
              renderKPIs();
              if (Modules.ActionCenter) {
                Modules.ActionCenter.renderEngagement();
                Modules.ActionCenter.render();
              }
              setTimeout(animateDashboard, 40);
            }).catch(function() {
              // Keep the currently loaded dashboard visible if the refresh fails.
            }).then(function() {
              window.MWAActionCenterReloading = false;
            });
          }
          [90, 500, 1200].forEach(function(delay) {
            setTimeout(animateDashboard, delay);
          });
        });
        ...
        if (!window.MWAPresenceRefreshTimer) {
          window.MWAPresenceRefreshTimer = setInterval(function() {
            var classPage = document.getElementById('page-classlist');
            var profilePage = document.getElementById('page-studentprofile');
            var visible = classPage && classPage.classList.contains('active') ||
              profilePage && profilePage.classList.contains('active');
            if (!visible || typeof window.MWAReloadData !== 'function' || window.MWAPresenceReloading) return;
            window.MWAPresenceReloading = true;
            window.MWAReloadData().catch(function() {
              // Keep the last known presence state when a refresh is unavailable.
            }).then(function() {
              window.MWAPresenceReloading = false;
            });
          }, 60000);
        }
        return dashboard;
